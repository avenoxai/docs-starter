...
Every trade on Slinq includes a dynamic fee that starts at 1% but adjusts based on market conditions. These fees are managed by a **hook** — a smart contract plugin built into PancakeSwap V4 that runs automatically on every trade.
...
<Tip>
Both scenarios give creators significant ongoing income. The 10% first-buyer reward incentivizes people to launch tokens they discover, creating a healthy ecosystem.
</Tip>
...
During the first 30 seconds after launch, a premium is added to **buy orders only** to protect against sniper bots. The premium decays using a **parabolic (x²) curve**:
...
<Accordion title="What's the maximum fee I could ever pay?">
  The absolute maximum hook fee is **5%**, which only happens during extreme volatility combined with MEV protection at launch. Under normal conditions, you'll pay 1-2%.
</Accordion>
